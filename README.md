...
O sistema realiza a autenticação no endpoint da API, consome os dados disponibilizados, aplica regras de tratamento e organização conforme a necessidade do gestor responsável e entrega um relatório padronizado, confiável e pronto para análise. Todo o desenvolvimento foi realizado em PYTHON, manipulação de dados usando PANDAS utilizando bibliotecas como REQUESTS para integração com API REST e JSON para manipulação e estruturação das informações, com VERSIONAMENTO controlado por meio do GitHub, garantindo rastreabilidade e organização do código.
